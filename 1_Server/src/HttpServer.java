...
class HttpServer {
    ...
    private void process(Request request, Response response) throws IOException {
        ...
        if (file.isFile()) {
            ...
            if (request.getHeader("If-None-Match") != null
                    && request.getHeader("If-None-Match").contains(eTag)){
    ...
    private String getETag(byte[] body) {
        return Integer.toHexString(Arrays.hashCode(body));
    }
    ...
    private Request readRequest(Socket socket) throws IOException {
        ...
        return new Request(URLDecoder.decode(new String(readInputStream(inputStream, true)), StandardCharsets.UTF_8.name()));
    }
